...
import { ATTACHMENT_CACHE_DIR, ensureDir, mimeToExt, sanitizeFilename, toMarkdownFilePath, toMarkdownImagePath, } from './mediaCache.js';
...
async function getAttachmentBuffer(att) {
    if (att.url.startsWith('data:')) {
        ...
        if (!match)
            throw new Error(`Formato data URL inválido no anexo "${att.name}"`);
        return { buffer: Buffer.from(match[2], 'base64'), mimeType: match[1] };
    }
    return fetchAttachmentBinary(att.url);
}
function saveAttachmentFile(cardId, index, name, buffer, mimeType) {
...
export async function materializeAttachment(cardId, index, att) {
...
export async function materializeAllAttachments(cardId, attachments) {
    const content = [];
