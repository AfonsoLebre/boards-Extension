...
import { addCardChecklist, updateCardChecklist, deleteCardChecklist, addChecklistItem, updateChecklistItem, deleteChecklistItem, getChecklistsList, memberHasIcon, } from './checklistApi.js';
...
const LIST_CARDS_HEADER = 'NOTA: Esta é uma listagem resumida. Para mostrar o conteúdo COMPLETO de um cartão (descrições, checklists, anexos, comentários, histórico), usa get_card com o card_id. A AI deve reproduzir o output de get_card integralmente, sem resumir.';
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        ...
        },
        {
            ...
            description: 'Lista colunas e cartões de um projeto (título, ID, coluna, prioridade, membros). NÃO inclui descrições, checklists, anexos nem comentários. Para mostrar o conteúdo COMPLETO de um cartão ao utilizador, usa get_card com o card_id.',
            inputSchema: {
                type: 'object',
                properties: {
        ...
        {
            name: 'search_cards',
            description: 'Procura cartões por título ou descrição. Devolve correspondências com ID. Para mostrar o conteúdo COMPLETO ao utilizador, usa get_card com o card_id encontrado.',
            inputSchema: {
                type: 'object',
                properties: {
        ...
        },
        {
            ...
            description: 'Adiciona um ou mais membros ao cartão. Usa o email de um participante do projeto. Para adicionar vários de uma vez, usa add_member_emails.',
            inputSchema: {
                type: 'object',
                properties: {
                    card_id: { type: 'number', description: 'ID do cartão' },
        ...
        {
            name: 'archive_card',
            description: 'Arquiva um cartão. O cartão deixa de aparecer no quadro ativo mas mantém-se acessível via get_card. Requer confirm: true.',
            inputSchema: {
                type: 'object',
                properties: {
                    card_id: { type: 'number', description: 'ID do cartão' },
    ...
    try {
        switch (name) {
            ...
            case 'search_cards': {
                ...
                const lines = [LIST_CARDS_HEADER, `**${results.length} resultado(s):**`];
            ...
            case 'get_card': {
                ...
                const priorityMap = { low: 'Baixa', normal: 'Normal', high: 'Alta', critical: 'Critica' };
                ...
                if (card.start_date)
                    header.push(`DATA INICIO: ${card.start_date}`);
                if (card.due_date)
                    header.push(`DATA FIM: ${card.due_date}`);
