...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        ...
        {
            name: 'get_card',
            description: 'OBRIGATÓRIO para mostrar um cartão ao utilizador. Devolve TODO o conteúdo: título, descrições completas, checklists, anexos, membros, etiquetas, datas, prioridade, coluna, comentários e histórico. A AI deve apresentar este output integralmente ao utilizador — sem resumir, truncar ou omitir conteúdo.',
            inputSchema: {
                type: 'object',
                properties: {
                    card_id: { type: 'number', description: 'ID do card' },
    ...
    try {
        switch (name) {
            ...
            case 'get_card': {
                ...
                // Secção 3: Datas
                if (card.start_date)
                    sections.push(`DATA INICIO: ${card.start_date}`);
                if (card.due_date)
                    sections.push(`DATA FIM: ${card.due_date}`);
